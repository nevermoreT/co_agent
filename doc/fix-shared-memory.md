...
### 根因
1. prompt 包含换行符和特殊字符（如 `---`）
2. Windows PTY 使用 `cmd.exe /c "claude ..."` 形式执行
3. 特殊字符被 shell 解析为选项，导致命令失败

错误日志：
```
[minimal-claude] JSON parse error for line: error: unknown option '---'
```

### 修复
改用数组形式传递参数给 PTY，而不是拼接命令字符串：
```javascript
// 之前（有问题）
...
- 修复 logger.js 格式化问题
- 分析 assistant 消息流程
- 修复 CLI 参数传递问题：改用数组形式避免 shell 解析错误
